    ...
    while (1)
    {
        if (timer.interruptAvailable())
        {
            ...
            if (message_length)
            {
                ...
                //sscanf(buffer, "%f,%f\n", &elbow_reference, &wrist_reference);
                sscanf(buffer, "%f,%f,%f,%f,%f,%f,%f,%f\n"
                , &elbow_gains[0], &elbow_gains[1], &elbow_gains[2], &elbow_reference // Elbow position and gains
                , &wrist_gains[0], &wrist_gains[1], &wrist_gains[2], &wrist_reference); // Wrist position and gains
            ...
            printf("%.2f,%.2f,%.2f,%.2f\n", elbow_reference, wrist_reference,elbow_measurement,wrist_measurement);
        }
    }
}
